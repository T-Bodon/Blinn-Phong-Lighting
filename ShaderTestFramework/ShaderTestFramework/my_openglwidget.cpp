...
void My_OpenGLWidget::generateGeometry() {
	...
	vec4* normals = new vec4[6];	// One normal for each face
...
void My_OpenGLWidget::rotateDown() {
	makeCurrent();
	downAngle -= 5.0f * 3.14159f / 180.0f;
	repaint();
}

void My_OpenGLWidget::rotateLeft() {
	makeCurrent();
	leftAngle -= 5.0f * 3.14159f / 180.0f;
	repaint();
}
...
void My_OpenGLWidget::resizeGL(int width, int height) {
	...
	float nearZ = 0.1f;
	float farZ = 10.0f;
	float top = nearZ * tanf(60.0f * 3.14159 / 180.0f);
	float right = top * aspectRatio;

	projectionMatrix = mat4(vec4(nearZ / right, 0.0f, 0.0f, 0.0f), vec4(0.0f, nearZ / top, 0.0f, 0.0f), vec4(0.0f, 0.0f, (-farZ + nearZ) / (farZ - nearZ), -1.0f), vec4(0.0f, 0.0f, (-2.0f * farZ * nearZ) / (farZ - nearZ), 0.0f));
}
